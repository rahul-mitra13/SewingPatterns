...
HalfedgeData<double> computeWaleOneForm(VertexPositionGeometry& globalGeometry, EdgeLengthGeometry& gluedGeometry, Model& model, 
    ...
    try {
        ...
        //         pathIntegral += hePath[k] * sigma[k];
        //     }
        //     if (i == 0) model.addConstr(pathIntegral == period * (-22));
        //     else model.addConstr(pathIntegral == period * (22));
        // }
...
std::tuple<CornerData<double>, EdgeData<double>> computeWaleStripeInfo(VertexPositionGeometry& globalGeometry, EdgeLengthGeometry& gluedGeometry, 
    ...
    for (int i = 0; i < globalBdyConditions.waleBdyPathConstraints.size() - 1; i++){
        std::vector<double> path = globalBdyConditions.waleBdyPathConstraints[i];
        double integral = 0.;
        double length = 0.;
        double integerVal = 0;
    ...
    //convert the global singular edges to glued singular 
    ...
    //before computing curl, do Laplacian smoothing of courseOneFormGrad 
    int num_iters = 1;
    for (int i = 0; i < num_iters; i++){
        for (Face f : gluedGeometry.mesh.faces()){
            Vector3 sum;
            for (Face n : f.adjacentFaces()){
                sum += courseOneFormGrad[n.getIndex()];
            }
            courseOneFormGrad[f.getIndex()] = sum / 3.;
            courseOneFormGrad[f.getIndex()] = courseOneFormGrad[f.getIndex()].normalize();
        }
    }

    VertexData<double> waleVertexCurl = computeVertexCurl(globalGeometry, gluedGeometry, courseOneFormGrad, gluedOneRingMap,
    ...
    psMesh.addVertexScalarQuantity("wale vertex curl", waleVertexCurl);
    psMesh.addEdgeScalarQuantity("wale edge curl", waleEdgeCurl);
...
std::tuple<HalfedgeData<double>, double> computeCourseOneForm(VertexPositionGeometry& globalGeometry, EdgeLengthGeometry& gluedGeometry, Model& gbModel, 
    ...
    try {
        ...
        for (Face f : gluedMesh.faces()){
            ...
            //normalize the comparison gradient
            // double length = sqrt(pow(comparisonGrad[f.getIndex()][0], 2) + pow(comparisonGrad[f.getIndex()][1], 2) + 
            //                         pow(comparisonGrad[f.getIndex()][2], 2));
            // comparisonGrad[f.getIndex()][0] = comparisonGrad[f.getIndex()][0]/length;
            // comparisonGrad[f.getIndex()][1] = comparisonGrad[f.getIndex()][1]/length;
            // comparisonGrad[f.getIndex()][2] = comparisonGrad[f.getIndex()][2]/length;
