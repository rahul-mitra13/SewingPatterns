...
std::tuple<HalfedgeData<double>, double> computeWaleOneForm(VertexPositionGeometry& globalGeometry, EdgeLengthGeometry& gluedGeometry, Model& model, 
    ...
    try {
        ...
        std::vector<GRBConstr> grbEdgePathConstraints;
        for (int i = 0; i < edgePathConstraints.size(); i++){
            ...
            for (int j = 0; j < gluedMesh.nEdges(); j++){
                if (path[j] != 0) {
                    Edge e = gluedMesh.edge(j);
                    Halfedge he = e.halfedge();
                    ensure(he.isInterior());
                    int heIndex = he.getIndex();
                    pathIntegral += path[j] * sigma[heIndex];
                }
            }
            grbEdgePathConstraints.push_back(model.addConstr(pathIntegral == period * edgePathConstraints[i].second));
        }
...
std::tuple<CornerData<double>, EdgeData<double>> computeWaleStripeInfo(VertexPositionGeometry& globalGeometry, EdgeLengthGeometry& gluedGeometry, 
    ...
    */
    ...
    //     EdgeData<double> pathVisu(gluedGeometry.mesh, 0.0);
    //     for (Edge e : gluedGeometry.mesh.edges())
    //         pathVisu[e] = path[e.getIndex()];
    //     psMesh.addEdgeScalarQuantity("path"+std::to_string(pathCount++), pathVisu);
    // }
...
std::tuple<CornerData<double>, EdgeData<double>> computeCourseStripeInfo(VertexPositionGeometry& globalGeometry, EdgeLengthGeometry& gluedGeometry, 
    ...
    int nSites = std::round(avgTotalMeasure / period);
    ...
    //     pairedSites = alignmentOptions.pairedSites;
    // }

    std::vector<std::pair<SurfacePoint, SurfacePoint>> pairedSites;

    if (opts.inputCourseSingPath.empty()) {
        ...
        P("Reading course singularity positions from file");
        std::ifstream in(opts.inputCourseSingPath);
        std::string line;
        while (std::getline(in, line)) {
            std::stringstream ss(line);
            int iEdgePos, iEdgeNeg; double tEdgePos, tEdgeNeg;
            if (!(ss >> iEdgePos >> tEdgePos >> iEdgeNeg >> tEdgeNeg))
                throw std::runtime_error("Parse error in line: " + line);
            Edge edgePos = globalMesh.edge(iEdgePos), edgeNeg = globalMesh.edge(iEdgeNeg);
            pairedSites.push_back({SurfacePoint(edgePos, tEdgePos), SurfacePoint(edgeNeg, tEdgeNeg)});
        }
    }

    // Write paired singularities to file
    // Not that surface points are on edges, not faces
    // Format is: edge+, tEdge+, edge-, tEdge-
    if (!opts.outputCourseSingPath.empty()) {
        std::ofstream out(opts.outputCourseSingPath);
        for (const auto &[posSP, negSP] : pairedSites) {
            out << posSP.edge.getIndex() << " " << posSP.tEdge << " ";
            out << negSP.edge.getIndex() << " " << negSP.tEdge << std::endl;
        }
    }
    ...
    for (int i = 0; i < singularEdges.size(); i++){
        ...
        if (faces.size() != 0 && code != -1){
            ...
            problemSPs.push_back(pairedSites[i].first.interpolate(globalGeometry.vertexPositions));
            problemSPs.push_back(pairedSites[i].second.interpolate(globalGeometry.vertexPositions));
    ...
    //first solve the model without any homology generators 
    ...
    for (std::vector<double> path : homologyGenerators){
        ...
        std::cout << "homology integer = " << round(integral/period) << std::endl;
        edgePathConstraints.push_back(std::make_pair(path, round(integral/period)));
    }

    //reset the edgePathConstraints
    model.setEdgePathConstraints(edgePathConstraints);
    //Now recompute with the generators set
    //first solve the model without any homology generators 
    std::tie(gluedSigmaTilde, currObj) = computeCourseOneForm(globalGeometry, gluedGeometry, model, vertexMap, G, psMesh);
    //plot the stripes
